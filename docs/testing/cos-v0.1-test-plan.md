...
5. **ADB-001** - db shell am broadcast -a com.example.cos.action.{SEED|NARODZINY|DOJRZEWANIE|NOWA_KOMORKA} odwzorowuje sekwencje etapow (manual/automation).
...
- Manual stage control: db shell am broadcast -a com.example.cos.action.SEED (reset lancucha), ...NARODZINY, ...DOJRZEWANIE, ...NOWA_KOMORKA, oraz db shell am broadcast -a com.example.cos.action.SET_STAGE --es com.example.cos.extra.STAGE <BUD|MATURE> dla pojedynczych etapow.
