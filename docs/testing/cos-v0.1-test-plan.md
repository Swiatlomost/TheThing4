...
5. **SEC-ADB-001** - Próba zewnętrznego `adb shell am broadcast` bez uprawnienia jest odrzucona; scenariusz przechodzi po stronie instrumentacji/app (posiada permission `com.example.cos.permission.CONTROL_LIFECYCLE`).
...
- Manual stage control: UI przyciski LIFE-001..004 PASS; `adb shell am broadcast` bez uprawnienia odrzucone (SEC-ADB-001 NEGATIVE PASS); instrumentacja posiadająca permission `com.example.cos.permission.CONTROL_LIFECYCLE` potwierdza pozytywne przejście sekwencji etapów.
